...
static mock_call_t *_mock_arguments_create(size_t arguments_count, va_list arguments)
{
  mock_call_t *mock_arguments = _mock_call_alloc(arguments_count);
  if (mock_arguments == NULL) return NULL;

  unsigned int arg_idx;
  for (arg_idx = 0; arg_idx < arguments_count; arg_idx++) {
    size_t arg_type_size = va_arg(arguments, size_t);
    switch (arg_type_size) {
    case 1 ... 4: mock_arguments->value[arg_idx] = (mock_argument_t)va_arg(arguments, int); break;
    case 8: mock_arguments->value[arg_idx] = (mock_argument_t)va_arg(arguments, uint64_t); break;
    default:
      _mock_call_free(mock_arguments);
      return NULL;
    }
  }
  return mock_arguments;
...
{
    mock_call_t *mock_arguments = _mock_arguments_create(arguments_count, arguments);
