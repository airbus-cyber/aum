...
static mock_call_t *_mock_arguments_create(size_t arguments_count, va_list arguments)
{
  ...
  for (arg_idx = 0; arg_idx < arguments_count; arg_idx++) {
    ...
    case 1 ... 4:
        mock_arguments->value[arg_idx] = (mock_argument_t) va_arg(arguments, int);
        break;
    case 8:
        mock_arguments->value[arg_idx] = (mock_argument_t) va_arg(arguments, uint64_t);
        break;
    // TODO here, we are not really sure this is a va_list => the code needs a rework. This choice should be made earlier, while we still know the type of the function
    //      or generic mock functions should be generated by clang
    case sizeof(va_list):
        printf("WARNING: Unable to store va_list argument. It is ignored... ");
        mock_arguments->value[arg_idx] = (mock_argument_t) NULL;
        break;
    default:
